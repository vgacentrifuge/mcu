...
#define KEY_WIDTH                                                              \
  ((KEYPAD_WIDTH - KEY_SPACING * (KEYPAD_COLS - 1)) / KEYPAD_COLS)
#define KEY_HEIGHT                                                             \
  ((KEYPAD_HEIGHT - KEY_SPACING * (KEYPAD_ROWS - 1)) / KEYPAD_ROWS)
...
const char *keySymbols[KEYPAD_ROWS * KEYPAD_COLS] = {[KEY_UP] = "^",
                                                     [KEY_LEFT] = "<",
                                                     [KEY_RIGHT] = ">",
                                                     [KEY_DOWN] = "V",
...
void keypad_sample() {
  ...
  for (int y = 0; y < KEYPAD_ROWS; y++) {
    for (int x = 0; x < KEYPAD_COLS; x++) {
      ...
      bool hover = localX >= 0 && localX < KEY_WIDTH && localY >= 0 &&
                   localY < KEY_HEIGHT;
      ...
      bool keyPressing =
          keyboardAlternatives[index] && IsKeyDown(keyboardAlternatives[index]);
...
bool keypad_keypressed(int key) { return key_down_time[key] == 1; }
...
void keypad_draw() {
  ...
  for (int y = 0; y < KEYPAD_COLS; y++) {
    for (int x = 0; x < KEYPAD_ROWS; x++) {
      ...
      DrawRectangle(x * KEY_STRIDE_X, y * KEY_STRIDE_Y, KEY_WIDTH, KEY_HEIGHT,
                    color);
      if (keySymbols[index])
        DrawText(keySymbols[index], x * KEY_STRIDE_X, y * KEY_STRIDE_Y,
                 KEY_TEXT_SIZE, GRAY);
    }
  }
  EndMode2D();
